<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="预约失信次数" prop="yuyueshixincishu">
          <el-input v-model="ruleForm.yuyueshixincishu" 
              placeholder="预约失信次数" clearable  :readonly="ro.yuyueshixincishu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="预约失信次数" prop="yuyueshixincishu">
              <el-input v-model="ruleForm.yuyueshixincishu" 
                placeholder="预约失信次数" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="是否禁用" prop="shifoujinyong">
          <el-select v-model="ruleForm.shifoujinyong" placeholder="请选择是否禁用">
            <el-option
                v-for="(item,index) in shifoujinyongOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="是否禁用" prop="shifoujinyong">
	      <el-input v-model="ruleForm.shifoujinyong"
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xingming = true;
          ...
          if(o=='yuyueshixincishu'){
            this.ruleForm.yuyueshixincishu = obj[o];
	    this.ro.yuyueshixincishu = true;
            continue;
          }
          if(o=='shifoujinyong'){
            this.ruleForm.shifoujinyong = obj[o];
	    this.ro.shifoujinyong = true;
            continue;
          }
          if(o=='riqi'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.shifoujinyongOptions = "已禁用,未禁用".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `shixinmingdan/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `shixinmingdan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shixinmingdanCrossAddOrUpdateFlag = false;
                  this.parent.search();
